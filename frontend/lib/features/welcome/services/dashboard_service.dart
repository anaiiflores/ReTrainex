...
import '../../routines/models/routine_model.dart' as r;        // Alias `r` para evitar conflicto con RoutineStatus de dashboard
...
class DashboardService {
  ...
  Future<DashboardModel> getDashboardData() async {
    ...
    final routinesFuture = RoutineService().getWeeklyRoutines(); // Carga rutinas de la semana actual
    ...
    final weeklyTotal = routines.length; // Total de rutinas planificadas esta semana
    final weeklyCompleted =
        routines.where((rt) => rt.status == r.RoutineStatus.completed).length; // Cuántas están completadas
    ...
    return DashboardModel(
      ...
      status: RoutineStatus.active,         // Estado activo: muestra el dashboard completo
      ...
      weeklyCompletedSessions: weeklyCompleted, // Calculado a partir de RoutineService
      ...
      nextSession: const NextSessionModel(
        date: '20 OCT',            // Fecha de la próxima sesión con el fisio
        time: '10:30 AM',          // Hora de la cita
        durationMinutes: 15,       // Duración estimada de la sesión
        type: 'FISIOTERAPIA',      // Tipo de sesión (con el fisio, no ejercicio propio)
      ),
      ...
      assignmentTitle: 'Nuevo tratamiento de ejercicios', // Título de la nueva rutina asignada
  ...
  Future<List<CalendarSessionModel>> getCalendarSessions() async {
    ...
    final completedDays = <int>{}; // Set de números de día (1=Lun…7=Dom) con sesión completada
    for (final rt in weeklyRoutines) {
      if (rt.status == r.RoutineStatus.completed) {
        completedDays.add(rt.weekday); // Guarda el día de la semana de cada rutina completada
      }
    }
    ...
    final start = DateTime(now.year, now.month - 1, 1);   // Primer día del mes anterior
    final end = DateTime(now.year, now.month + 2, 0);      // Último día del mes siguiente (día 0 = último del mes anterior)
    ...
    for (var d = start; d.isBefore(end); d = d.add(const Duration(days: 1))) {
      ...
      if (d.weekday == DateTime.monday) {         // weekday == 1
      ...
      } else if (d.weekday == DateTime.wednesday) { // weekday == 3
        sessions.add(CalendarSessionModel(
          date: d,
      ...
      } else if (d.weekday == DateTime.friday) {   // weekday == 5
        sessions.add(CalendarSessionModel(
          date: d,
